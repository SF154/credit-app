...
import { ExpandableText } from '@/components/applications/ExpandableText'
import type { ApplicationWithTemplate } from '@/types'

async function getRejectedApplications(companyId: string) {
...
export default async function RejectedPage() {
  ...
  const applications = await getRejectedApplications(profile.company_id)

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-zinc-900">Rejected Applications</h1>
        <p className="text-sm text-zinc-500 mt-0.5">
          Applications that have been reviewed and rejected.
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50">
                ...
                <th className="text-left px-4 py-3 font-medium text-zinc-500">Rejection Date</th>
                ...
                <tr key={app.id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-4 py-3 max-w-xs">
                    {app.rejection_reason ? (
                      <ExpandableText text={app.rejection_reason} />
                    ) : (
